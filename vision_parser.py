...
Day 2 deliverable: bridges the VLM's extracted-features JSON (from
vision_extractor.py) into the exact input shape parser.py already expects
-- so the rest of the pipeline (tokenizer.py onward) does not need to
change at all. Reuses parser.py's own validation (parse_input) rather than
duplicating it, consistent with the project's principle that generative
...
Day 3 confirmation screen where the user enters it manually).
"""
...
if __name__ == "__main__":
    # Quick manual test using a sample vision_extractor.py-shaped output.
    sample_vlm_output = {
        "features": [
        ...
        "material": "steel",       # deliberately lowercase to test normalization
        "tolerance": "± 0.02 mm",  # deliberately messy to test normalization
    }
