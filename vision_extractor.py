...
Sends ANY image showing machining features -- a formal 2D engineering
drawing, a CAD screenshot, or a plain photo where machining features
(holes, facing marks, turned sections, etc.) are visible -- to Groq's
vision-capable model, and returns a structured JSON dict of extracted
machining features, material, and tolerance.
...
def build_prompt(feature_vocabulary: list[str]) -> str:
    ...
    return (
        "You are inspecting an image related to a manufactured/machined "
        "part. The image could be any of the following: a formal 2D "
        "engineering drawing (orthographic views, dimension lines, title "
        "block), a CAD software screenshot, a hand sketch, or a plain "
        "photograph of a physical part or a paper drawing. Treat all of "
        "these as valid inputs.\n\n"
        "Identify every machining feature visible in the image -- "
        "features that would result from lathe operations (e.g. turning, "
        "facing, threading, knurling) or milling operations (e.g. "
        "drilling, slotting, pocketing), or that are directly visible as "
        "geometric marks on the part or drawing. "
        ...
        "guessing a new name.\n\n"
        "If a formal title block is present, also read it for material "
        "and tolerance. If the image is a photo or sketch without a "
        "title block, use null for material and tolerance rather than "
        "guessing.\n\n"
...
def extract_features_from_image(image_path: str) -> dict:
    """
    Sends the image to the vision model and returns the parsed JSON dict.
    Raises ValueError if the model does not return valid JSON matching the
    expected shape -- the caller decides how to handle that (re-prompt
    once, or fall back to manual dropdown entry).
    """
